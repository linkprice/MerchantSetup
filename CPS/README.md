...
create table lpinfo(
    ...
    lpinfo varchar(560),		 				    # 링크프라이스 트래킹 코드 (쿠키명 LPINFO) 
    user_agent varchar(1000), 					    # 구매자의 User Agent 값
...
| linkprice.lpinfo               | 링크프라이스에서 유입된 트래킹 코드(=LPINFO 쿠키)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | varchar(560)    |
...
상품금액의 비율에 따라 쿠폰가를 차감 계산

```
# 상품1 비율 계산
5000 / (5000 + 10000 + 7000) = 0.227273 (소수점 6자리까지 계산)

# 상품1 할인 비율 계산
5000 - (5000 * 0.227) = 3,864

# 상품2 비율 계산
10000 / (5000 + 10000 + 7000) = 0.454545 (소수점 6자리까지 계산)

# 상품2 할인 비율 계산
10000 - (5000 * 0.454) = 7,727

# 상품3 비율 계산
7000 / (5000 + 10000 + 7000) = 0.318182 (소수점 6자리까지 계산)

# 상품3 할인 비율 계산
7000 - (5000 * 0.318) = 5,409
```

22,000원에서 5000원 쿠폰을 사용 시, **총 17,000원** 전송
...
5000 - ((5000 * 0.2) + (5000 * 0.227273)) = 2,864

# 상품2 정액 쿠폰의 비율 계산
10000 / (5000 + 10000 + 7000) = 0.454545 (소수점 6자리까지 계산)
# 상품2 정률, 정액 계산하여 할인 적용
10000 - ((10000 * 0.2) + (5000 * 0.454545)) = 5,727

# 상품3 정액 쿠폰의 비율 계산
7000 / (5000 + 10000 + 7000) = 0.318182 (소수점 6자리까지 계산)
# 상품3 정률, 정액 계산하여 할인 적용
7000 - ((7000 * 0.2) + (5000 * 0.318182)) = 4,009
```
