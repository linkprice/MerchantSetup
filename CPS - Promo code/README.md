...
|    필드명    | 타입(Byte)  | 필수유무 |                          설명                          |
| :----------: | :---------: | :------: | :----------------------------------------------------: |
|  event_code  | string(20)  |   필수   |      링크프라이스에서 발급한 고유 이벤트 코드<a name="link1">[1]</a>      |
...
<sup>[1](#link1)</sup>: 이벤트 코드(event_code) - 링크프라이스와 광고주가 협의 하여 생성하는 고유코드 이며, 고정값 입니다.  <br>Ex) merchant_promocode,   merchant_sale
[^2]: 할인 코드(promo_code) - 실제 구매자가 사용하는 할인코드로, 광고주가 발급후 링크프라이스에 전달해야 하는 코드 입니다.   Ex) LPpromo_code01, LPpromo_code02, LPpromo_code03, LPpromo_code04, LPpromo_code05
[^3]: User Agent 예시 : Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.36"
[^4]: PC웹 실적 - "PC"   /   모바일 웹 실적 - "MOBILE"    /   모바일 앱 실적 - "APP"
